...
void xlog(const char* name, GLenum em)
{
...
void glrenderer(char* row, int w, int h)
{
    GLuint texture, renderBuffer, frameBuffer;
    xlog("Version", GL_VERSION);
    xlog("Vendor", GL_VENDOR);
    xlog("Renderer", GL_RENDERER);
...
JNIEXPORT void JNICALL Java_com_panda_org_pbufferwrapper_getWrapperGLImage(JNIEnv *env, jclass cls, jobject bitmap)
{

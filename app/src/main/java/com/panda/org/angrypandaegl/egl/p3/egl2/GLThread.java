...
public class GLThread extends Thread implements ISurface{
    ...
    private ArrayBlockingQueue<GLThread.Event> eventQueue;
    private final List<GLSurface> outputSurfaces;
    private boolean rendering;
    private boolean isRelease;

    private IRenderer renderer;
    public GLThread(IRenderer re) {
        setName("GLThread-" + getId());
        outputSurfaces = new ArrayList<>();
        rendering = false;
        isRelease = false;

        renderer=(PBufferRenderer)re;
    ...
    * */
    ...
    public void addSurface(final GLSurface surface){
        GLThread.Event event = new GLThread.Event(GLThread.Event.ADD_SURFACE);
        event.param = surface;
    ...
    public void removeSurface(final GLSurface surface){
        GLThread.Event event = new GLThread.Event(GLThread.Event.REMOVE_SURFACE);
        event.param = surface;
        if(!eventQueue.offer(event))
            Log.e(TAG,"queue full");
    }
    ...
    public void startRender(){
        if(!eventQueue.offer(new GLThread.Event(GLThread.Event.START_RENDER)))
            Log.e(TAG,"queue full");
    ...
    public void stopRender(){
        if(!eventQueue.offer(new GLThread.Event(GLThread.Event.STOP_RENDER)))
            Log.e(TAG,"queue full");
    }

    public boolean postRunnable(Runnable runnable){
        GLThread.Event event = new GLThread.Event(GLThread.Event.RUNNABLE);
    ...
    public void requestRender(){
        eventQueue.offer(new GLThread.Event(GLThread.Event.REQ_RENDER));
    }
    ...
    private void initEGL() {
        ...
        // 设置默认的上下文环境和输出缓冲区(小米4上如果不设置有效的eglSurface后面创建着色器会失败，可以创建一个默认先的eglSurface)
        ...
        while(!isRelease){
            try {
                ...
                * */
                ...
                switch(event.event){
                    case GLThread.Event.ADD_SURFACE: {
                        // 创建eglSurface
                    ...
                    case GLThread.Event.REMOVE_SURFACE: {
                        GLSurface surface = (GLSurface)event.param;
                    ...
                    case GLThread.Event.START_RENDER:
                        rendering = true;
                        break;
                    case GLThread.Event.REQ_RENDER: // 渲染
                        if(rendering) {
                            //onUpdate();
                            renderer.onSurfaceChanged(0,0);
                            render(); // 如果surface缓存没有释放(被消费)那么这里将卡住
                        }
                        break;
                    case GLThread.Event.STOP_RENDER:
                        rendering = false;
                        break;
                    case GLThread.Event.RUNNABLE:
                        /*
                        * 获取数据的
                        * */
                        ((Runnable)event.param).run();
                        break;
                    case GLThread.Event.RELEASE:
                        isRelease = true;
                        break;
                    default:
     ...
     */
    public void release(){
        if(eventQueue.offer(new GLThread.Event(GLThread.Event.RELEASE))){
